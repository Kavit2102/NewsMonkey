...
export default function Countries() {
  ...
  const { country, changeCountry } = useContext(CountriesContext);
  const [menuItem, setMenuItem] = useState(country);

  let textColor;
  const getCountry = async () => {
    try {
      let storedCountry = await localStorage.getItem("country");
      setMenuItem(storedCountry || "Country");
    } catch (error) {
  ...
  const handleMenuItemChange = async (selectedItem) => {
    try {
      ...
      await localStorage.setItem("code", selectedItem.code);
      changeCountry(selectedItem.code);
    } catch (error) {
